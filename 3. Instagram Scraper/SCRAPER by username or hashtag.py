...
def posts_by_user(user, tag = False):
    ...
    if tag:
        folder ="Collected Data by Hashtag/" + user + "/"
    try:
    ...
    if tag:
        filename = "Collected Data by Hashtag/" + user + ".csv"
    f = open(filename, "w")
    ...
    if tag:
        url = "http://www.instagram.com/tags/"+user
    ...
    while True:
        ...
        for link in links:
            if post in link and link not in post_links:
                i+=1
                post_data = find_data(link, user, tag, i)
                if post_data == None:
                    return #goes through this if date is before the event specified
                f.write(post_data)
                post_links.add(link)
        if before_links == len(post_links):
            print("Posts retrieved")
            return #see if theres error with the hashtag method
...
def find_data(post, user, tag, i=0):
    ...
    # # check date for dia de las velitas
    # if date_unsplit != "unknown":
    #     post_date = date_unsplit.split("-")
    #     post_date = [int(i) for i in post_date]
    #     if date(2019, 12, 6) > date(post_date[0], post_date[1], post_date[2]):
    #         return None
...
tag = input("Scrapping a hashatag? (True or False): ")
username = input("Instagram Username or Hashtag: ")
posts_by_user(username, tag)
winsound.Beep(380, 5000)